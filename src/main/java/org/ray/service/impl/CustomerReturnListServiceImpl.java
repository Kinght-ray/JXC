...
import org.ray.entity.CustomerReturnList;
import org.ray.entity.CustomerReturnListGoods;
import org.ray.entity.Goods;
import org.ray.repository.CustomerReturnListGoodsRepository;
import org.ray.repository.CustomerReturnListRepository;
		...
		return customerReturnListRepository.findAll(new Specification<CustomerReturnList>(){
			...
			public Predicate toPredicate(Root<CustomerReturnList> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
				...
				if(customerReturnList!=null){
					...
					if(customerReturnList.getBeginCustomerReturnDate()!=null){
						predicate.getExpressions().add(cb.greaterThanOrEqualTo(root.get("customerReturnDate"), customerReturnList.getBeginCustomerReturnDate()));
					}
					if(customerReturnList.getEndCustomerReturnDate()!=null){
						predicate.getExpressions().add(cb.lessThanOrEqualTo(root.get("customerReturnDate"), customerReturnList.getEndCustomerReturnDate()));
					}
				}
				return predicate;
			}
